...
main(){
	...
	cp sources/90-clusterhat.rules ~/mnt/etc/udev/rules.d/ && mkdir ~/mnt/etc/minicom && echo -e "pu port\t/dev/ttypi1" > ~/mnt/etc/minicom/minirc.p1 && echo -e "pu port\t/dev/ttypi2" > ~/mnt/etc/minicom/minirc.p2 && echo -e "pu port\t/dev/ttypi3" > ~/mnt/etc/minicom/minirc.p3 && echo -e "pu port\t/dev/ttypi4" > ~/mnt/etc/minicom/minirc.p4 && chroot ~/mnt systemctl set-default multi-user.target && chroot ~/mnt ln -fs /lib/systemd/system/getty@.service /etc/systemd/system/getty.target.wants/getty@ttyGS0.service && sed -i "s#^127.0.1.1.*#127.0.1.1\tcontroller#g" ~/mnt/etc/hosts && echo "controller" > ~/mnt/etc/hostname && echo -e "\nauto br0\niface br0 inet dhcp\nbridge_ports eth0\nbridge_stp off\nbridge_waitport 0\nbridge_fd 0" >> ~/mnt/etc/network/interfaces && echo -e "\nauto ethpi1\nallow-hotplug ethpi1\niface ethpi1 inet manual\npre-up brctl addif br0 ethpi1\nup ifconfig ethpi1 up" >> ~/mnt/etc/network/interfaces && echo -e "\nauto ethpi2\nallow-hotplug ethpi2\niface ethpi2 inet manual\npre-up brctl addif br0 ethpi2\nup ifconfig ethpi2 up" >> ~/mnt/etc/network/interfaces && echo -e "\nauto ethpi3\nallow-hotplug ethpi3\niface ethpi3 inet manual\npre-up brctl addif br0 ethpi3\nup ifconfig ethpi3 up" >> ~/mnt/etc/network/interfaces && echo -e "\nauto ethpi4\nallow-hotplug ethpi4\niface ethpi4 inet manual\npre-up brctl addif br0 ethpi4\nup ifconfig ethpi4 up" >> ~/mnt/etc/network/interfaces && cp clusterhat ~/mnt/sbin/clusterhat && chmod a+x ~/mnt/sbin/clusterhat && chroot ~/mnt rm -f mnt/etc/ssh/*key* && chroot ~/mnt apt-get -qy autoremove --purge && chroot ~/mnt apt-get clean
	if [ $? -ne 0 ]; then
